...
@section('content')
    ...
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Edit Material Category</h1>
                <p class="text-gray-600 mt-2">{{ $category->name }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('erp.raw-materials.categories.update', $category) }}">
            ...
            <div class="space-y-6 mb-6">
                <div>
                    ...
                    <input type="text" name="name" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 focus:outline-none" value="{{ old('name', $category->name) }}" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Description</label>
                    <textarea name="description" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 focus:outline-none" rows="3">{{ old('description', $category->description) }}</textarea>
                </div>
            </div>
